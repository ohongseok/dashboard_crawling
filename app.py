...
col2.metric("토탈 브랜드 수", f"{total_brands:,} 개")
col3.metric("최다 진행 브랜드", top_brand)
    ...
    for idx, manager in enumerate(TARGET_MANAGERS):
        with lead_cols[idx]:
            ...
            if m_daily_df.empty:
                ...
                st.metric("금일 완료 SKU", f"{daily_sku_sum:,} 개")
...
st.caption("스크롤을 내려 각 담당자별 상세 지표, 차트, 전체 데이터 로그를 확인하실 수 있습니다.")
st.markdown("---")

# 탭을 제거하고 담당자별로 위아래로 길게 나열합니다.
